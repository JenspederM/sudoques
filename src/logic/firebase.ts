 ...
 */
export async function saveGameState(userId: string, state: Omit<GameState, "lastUpdated">) {
 ...
 */
export async function loadGameState(userId: string): Promise<Omit<GameState, "lastUpdated"> | null> {
  const gameRef = doc(db, GAMES_COLLECTION, userId);
